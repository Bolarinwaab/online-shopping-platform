...
gcloud iam workload-identity-pools describe "${POOL_ID}" --location=global --project="${PROJECT_ID}" --format='value(name)' || {
...
gcloud iam workload-identity-pools providers describe "${PROVIDER_ID}" --workload-identity-pool="${POOL_ID}" --location=global --project="${PROJECT_ID}" --format='value(name)' >/dev/null
...
echo "WIF bootstrap complete. Configure GitHub Actions variables/secrets documented in docs/gcp-github-actions.md."
